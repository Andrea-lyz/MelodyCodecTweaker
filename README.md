...
**LHDC V5 native 补丁新增 PJZ110 16.0.10.501 适配（真机验证）**

- 快切等价补丁新增 `lhdcv5_quality_equals_pjz110_1610501` 整块签名：一加 13 升级 ColorOS 16.0.10.501 后，「音质优先」纯码率切档不再被判定为必须重建输出，消除切档卡顿 / 重建拥塞。
- 码率 branch 补丁 guard 与 16.0.9.401 同字节（`branch_plus_68_pjz110_1609401_1610501`），沿用既有模式即可命中。
- 一加 13（16.0.10.501）实机验证：码率 branch / 快切等价两条补丁状态均 `patched`，音质优先切档正常；适配矩阵新增该版本线（✅ 真机验证）。
