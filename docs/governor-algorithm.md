...
- **实验性开关（默认关闭）**：治理器作为实验性功能交付，运行时默认关闭（release 与
  debug 一视同仁）。用户可在模块诊断页「实验性：自动码率保护」开启——开关立即同步到
  蓝牙进程并跨重启持久化；关闭时立即清除所有激活的 cap、边界学习锁与恢复证据（码率
  回到设备能力上限），停止降级/恢复评估，choppy/BQR 仍照常记录供诊断。
