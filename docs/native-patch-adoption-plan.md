...
背景：宿主面板选择 LHDC 档位（音质优先 1000/900、连接优先 500、自适应）时，按
码率 branch（B）与快切等价（F）补丁的适配状态给出差异化反馈。现状仅写失败后按 B
弹「未适配」，F 完全不参与判定。
...
| ---- | ---- | ---- | ---- |
| patched | patched | 任意 LHDC 档位切换 | 静默放行 |
| patched | unsupported | 任意档位切换（1000/900/500/400/自适应） | 放行 + 预提醒（选中即弹，异常卡顿文案） |
| unsupported | 任意 | 固定 1000 / 900（音质优先） | 拒绝语义保留：写失败后「未适配，请联系开发者反馈」（900/1000 统一纳入） |
| unsupported | 任意 | 500 / 400 / 自适应 | 放行（不依赖 B） |
| 任意 | pending / 未知 | 任意 | 静默放行（不误伤） |

判定规则：

1. 拒绝只看 B，且仅限固定 1000/900 档（现状只拦 900，实施时统一 900+1000）；
2. 提醒只看 F（任何 LHDC 档位切换——F 缺失时纯码率切换会重建输出，实测为异常卡顿），
   且仅当 B 非 unsupported 时提醒，避免与「未适配」双 toast 叠加；
3. 未知一律放行；
4. 宿主新增 `fastSwitchUnsupported` 布尔（判定同构 `nativePatchUnsupported`：
   status=unsupported 且 patched=0 且 original=0）；
5. 预提醒时机 = 选中即弹，仅在真实切换时（重复选同档不弹）；每次切换都弹，不设会话去重。
